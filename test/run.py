...
def _make_testcase(testcase):
    ...
    def test_method(self):
        ...
        try:
            ...
            actual_output: List[int] = self._run_case(list_head=list_head, x=x)
        except TimeoutError:
...
class TestSolution(unittest.TestCase):
    ...
    @timeout(1)
    def _run_case(self, list_head: Optional[ListNode], x: int) -> List[int]:
        # ⚡ Give every testcase its own timeout instead of timing the whole file.
        head: Optional[ListNode] = self.__solution.partition(list_head, x)
        result: List[int] = []
...
class PartitionListTestResult(unittest.TextTestResult):
...
if __name__ == '__main__':
    unittest.main(
        verbosity=2,
        testRunner=unittest.TextTestRunner(resultclass=PartitionListTestResult)
    )
